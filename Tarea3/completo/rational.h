...
*/
template <class INT>
inline rational<INT>& rational<INT>::operator/=( const rational<INT>& nume ) {
    num() *= nume.den();
    den() *= nume.num();
    simplify();
    return *this;
...
*/
//  return double(x.num()) / double(x.den()) < double(y.num()) / double(y.den());
}
